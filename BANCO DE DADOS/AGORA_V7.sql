...
CREATE TABLE TB_PRODUTOS(
    ...
    VALOR_UNI_COMPRA DECIMAL(9,2),
    VALOR_UNI_VENDA DECIMAL(9,2), 
	DATA_CADASTRO DATETIME,
...
CREATE TABLE TB_CONTAS_PAGAR(
    ID_PAGAR INT PRIMARY KEY IDENTITY(1,1),
    ID_COMPRA INT,  
	DESCRICAO VARCHAR(50),
	CATEGORIA VARCHAR(30),
	VALOR_PAGAR DECIMAL(9,2),
	DATA_VENCIMENTO DATETIME, 
...
CREATE FUNCTION PROCURA_ID_GERAL_CPF_CNPJ (@CPF_CNPJ VARCHAR(20)) 
RETURNS INT
AS
BEGIN 
...
CREATE FUNCTION PROCURA_ID_GERAL_ID (@ID UNIQUEIDENTIFIER) 
RETURNS INT
AS
BEGIN 
	DECLARE @ID_GERAL INT

		IF (@ID) IN (SELECT ID_FUNC FROM TB_FUNCIONARIOS)
			BEGIN
				SET @ID_GERAL = (SELECT ID_GERAL FROM TB_PRINCIPAL_PESSOA WHERE ID_FUNC = @ID)
								 
			END
			
		ELSE IF (@ID) IN (SELECT CPF FROM TB_CLIENTES)
			BEGIN
				SET @ID_GERAL = (SELECT ID_GERAL FROM TB_PRINCIPAL_PESSOA WHERE ID_CLI = @ID)
				END
		ELSE IF (@ID) IN (SELECT CNPJ FROM TB_FORNECEDORES)
			BEGIN
				SET @ID_GERAL = (SELECT ID_GERAL FROM TB_PRINCIPAL_PESSOA WHERE ID_FORN = @ID)
			END
		ELSE IF (@ID) IN (SELECT CNPJ FROM TB_ESTABELECIMENTOS)
			BEGIN
				SET @ID_GERAL = (SELECT ID_GERAL FROM TB_PRINCIPAL_PESSOA WHERE ID_ESTAB = @ID)
				END
			RETURN @ID_GERAL
END
GO
...
CREATE PROCEDURE PROC_VENDA_ESTAB
	@ACAO CHAR(1), -- INSERIR(I), ALTERAR(A), EXCLUIR(E), PAGAR(P) 
	@COMANDA INT, 
	@ID_PRODUTO INT,
	@QUANTIDADE INT,
	...
	BEGIN 		
		...
		ELSE IF(@ACAO = 'A')
			BEGIN
				...
				WHERE ID_COMANDA_LISTA = @COMANDA
				AND	ID_PROD_LISTA = @ID_PRODUTO
			END
		ELSE IF(@ACAO = 'E')
				BEGIN
					DELETE FROM TB_LISTA_PRODUTOS
					WHERE ID_COMANDA_LISTA = @COMANDA
					AND ID_PROD_LISTA = @ID_PRODUTO
				END
				...
				BEGIN
					IF(@STATUS_ATUAL != 'D')
						...
						VALUES (@COMANDA,
								...
								GETDATE(),
								(CASE 
									WHEN @METODO_PAGAMENTO = 'DI' -- DINHEIRO
									THEN GETDATE()

									WHEN @METODO_PAGAMENTO = 'CR'  -- CREDITO
									THEN GETDATE() + 30

									WHEN @METODO_PAGAMENTO = 'DE'  -- DEBITO
									THEN GETDATE() + 1
								 END
									) 
								)
					...
					DELETE FROM TB_LISTA_PRODUTOS WHERE ID_COMANDA_LISTA = @COMANDA 
				END
		RETURN @COMANDA 
	END 
GO    
...
AS 
	BEGIN
		IF(@ACAO = 'IC') -- 'INSERIR COMPRA' NA COMPRA
			BEGIN 
				INSERT INTO TB_COMPRAS VALUES('C', GETDATE(), NULL)
...
AS 
	BEGIN  
		...
		ELSE IF(@ACAO = 'E')
			BEGIN
				DELETE FROM TB_LOGIN WHERE ID_LOGIN = (SELECT ID_LOGIN FROM TB_LOGIN WHERE USUARIO = @USUARIO)
			END
	END
GO     

CREATE PROCEDURE PROC_ALTERA_LOGIN 
	@CPF VARCHAR(14),
	@USUARIO VARCHAR(60),
	@SENHA VARCHAR(30) 
AS 
	BEGIN    
		DECLARE @ID_LOGIN INT = (SELECT ID_LOGIN FROM TB_LOGIN WHERE USUARIO = @USUARIO)
		DECLARE @CPF_ORIGINAL VARCHAR(14) = (SELECT CPF FROM TB_FUNCIONARIOS WHERE ID_LOGIN = @ID_LOGIN)
		IF(@CPF = @CPF_ORIGINAL)  
			BEGIN
				UPDATE TB_LOGIN 
				SET SENHA = @SENHA
...
AS
	BEGIN 	
		IF(@ACAO = 'I')
			BEGIN
				INSERT INTO TB_FUNCIONARIOS VALUES (NEWID(),@NOME,@SOBRENOME,@RG,@CPF,@DT_NASC,@SEXO,@CARGO,NULL, GETDATE())	
			END
		...
		ELSE IF(@ACAO = 'A')
			BEGIN
				...
				UPDATE TB_LOGIN
				SET ID_HIERARQUIA = (SELECT ID_HIERARQUIA FROM TB_HIERARQUIA WHERE CARGO = @CARGO)
				WHERE ID_LOGIN = (SELECT ID_LOGIN FROM TB_FUNCIONARIOS WHERE ID_FUNC = @ID_FUNC)
			END
...
AS
	BEGIN
		DECLARE @ID_GERAL INT = (SELECT DBO.PROCURA_ID_GERAL_CPF_CNPJ(@CPF_CNPJ))
			IF(@ACAO = 'I') -- INSERIR
				BEGIN
			...
			ELSE IF(@ACAO = 'A')	--ALTERAR				
					BEGIN 
				--	SET @ID_GERAL = (SELECT DBO.PROCURA_ID_GERAL_ID(@CPF_CNPJ))
						UPDATE TB_ENDERECO	
						SET  CEP = @CEP,
							 RUA = @RUA,							 
							 NUMERO = @NUMERO,
...
AS
	BEGIN
		...
		IF(@ACAO = 'I')
			BEGIN
				INSERT INTO TB_CLIENTES VALUES (NEWID(),@NOME,@SOBRENOME,@RG,@CPF,(FORMAT(@DT_NASC, 'dd/MM/yyyy')),@SEXO,NULL, GETDATE())	
			END
...
AS
	BEGIN
 		IF(@ACAO = 'I')
			BEGIN
				INSERT INTO TB_FORNECEDORES VALUES (NEWID(),@RAZAO_SOCIAL, @CNPJ, GETDATE())
			END
...
BEGIN
		...
		ELSE IF(@ACAO = 'E')
			BEGIN
				DELETE FROM TB_CONTAS_RECEBER WHERE ID_RECEBER = @ID_RECEBER 
			END
END
GO
...
BEGIN
		IF(@ACAO = 'I')
			BEGIN
				INSERT INTO TB_CONTAS_PAGAR (DESCRICAO,CATEGORIA,VALOR_PAGAR,DATA_VENCIMENTO) 
									VALUES(@DESCRICAO,@CATEGORIA,@VALOR_PAGAR,@DATA_VENCIMENTO) 
			END
...
AS
	BEGIN
		IF(@ACAO = 'I')
			BEGIN
				INSERT INTO TB_ESTOQUE VALUES (@ID_PRODUTO, @QUANTIDADE, GETDATE(), @DATA_VENCIMENTO)
			END
		ELSE IF(@ACAO = 'A')
			BEGIN
				UPDATE TB_ESTOQUE
...
AS 
	BEGIN
		...
		UPDATE TB_LOGIN
		SET ID_HIERARQUIA = @HIERARQUIA
		WHERE ID_LOGIN = (SELECT ID_LOGIN FROM TB_FUNCIONARIOS WHERE ID_FUNC = @ID_FUNCIONARIO)
...
AS 	
	BEGIN 	
		DECLARE @ID_COMPRA INT =(SELECT ID_COMPRA FROM TB_COMPRAS WHERE STATUS = 'Q'),
				@ID_PRODUTO INT,
				@QUANTIDADE INT 			
		
		DECLARE @VALOR_PAGAR DECIMAL(9,2)  = (SELECT SUM(SUBTOTAL) AS TOTAL 
											  FROM VW_COMPRA
											  WHERE ID_COMPRA = @ID_COMPRA)
		...
		WHILE @@FETCH_STATUS = 0
			...
			INSERT INTO TB_CONTAS_PAGAR	VALUES(@ID_COMPRA,NULL,NULL, @VALOR_PAGAR,GETDATE() + 30)		
	 END 
GO	  
...
AS
	...
	FROM TB_CONTAS_PAGAR PAGAR
		INNER JOIN TB_COMPRAS COMPRA ON COMPRA.ID_COMPRA  =  PAGAR.ID_COMPRA 
GO 
...
AS
	SELECT ID_COMPRA,
		   STATUS,	
		   (SELECT RAZAO_SOCIAL FROM TB_FORNECEDORES WHERE ID_FORN = ID_FORN_PROD) AS RAZAO_SOCIAL,
		   LISTA.ID_PRODUTO, 
...
AS
	SELECT	ID_COMPRA,
			(SELECT STATUS_COMANDA FROM TB_COMANDA WHERE ID_COMANDA = ID_COMANDA_LISTA) AS STATUS_COMANDA,
			ID_COMANDA_LISTA,
			NOME_PRODUTO,
			VALOR_UNI_VENDA,
			ID_PROD_LISTA,
			...
			DATA_COMPRA,
			DATA_SAIDA,
			(SELECT QUANTIDADE * VALOR_UNI_VENDA) AS SUBTOTAL
	FROM TB_LISTA_PRODUTOS LISTA
	INNER JOIN TB_PRODUTOS PRODUTO ON PRODUTO.ID_PRODUTO = LISTA.ID_PROD_LISTA
	INNER JOIN TB_COMANDA COMANDA ON COMANDA.ID_COMANDA = LISTA.ID_COMANDA_LISTA
GO
  
/* 
	----------------------------
	SELECT * FROM TB_COMANDA 
	SELECT STATUS_COMANDA FROM TB_COMANDA WHERE ID_COMANDA = 4480
	SELECT * FROM TB_LISTA_PRODUTOS  
	DELETE FROM TB_LISTA_PRODUTOS  
	--------------------------------

	INSERT INTO TB_FORNECEDORES VALUES (NEWID(), 'PADARIA FILHO DA ROSA','81.709.591/0001-72', GETDATE()) 

	INSERT INTO TB_PRODUTOS VALUES (  (SELECT ID_FORN FROM TB_FORNECEDORES WHERE RAZAO_SOCIAL = 'PADARIA FILHO DA ROSA' ),'COCA-COLA 2L', NULL, 'REFRIGERANTE', 5,8, GETDATE() )
	INSERT INTO TB_PRODUTOS VALUES (  (SELECT ID_FORN FROM TB_FORNECEDORES WHERE RAZAO_SOCIAL = 'PADARIA FILHO DA ROSA' ),'FARINHA 20K', 'FARINHA SEARA TIPO 2 20K - SEM GLUTEN', 'FARINHA', 50.60, NULL,GETDATE() )
	INSERT INTO TB_PRODUTOS VALUES (  (SELECT ID_FORN FROM TB_FORNECEDORES WHERE RAZAO_SOCIAL = 'PADARIA FILHO DA ROSA' ),	'DORITOS 720G', NULL, 'SALGADINHOS', 9, 12.98,GETDATE() )
	 
	EXEC PROC_COMPRA_ESTAB IC,NULL,NULL,NULL
	EXEC PROC_COMPRA_ESTAB II,1000,1000,200 
	EXEC PROC_COMPRA_ESTAB II,1000,1002,200 
	EXEC PROC_COMPRA_ESTAB II,1000,1001,200 
	EXEC PROC_COMPRA_ESTAB C,1000,NULL,NULL    

	EXEC PROC_VENDA_ESTAB 'I',3245, 1000, 500,NULL, NULL, NULL, NULL
	EXEC PROC_VENDA_ESTAB 'I',3245, 1001, 100,NULL, NULL, NULL, NULL
	EXEC PROC_VENDA_ESTAB 'I',3245, 1002, 200,NULL, NULL, NULL, NULL

	EXEC PROC_VENDA_ESTAB 'I',4480, 1000, 540,NULL, NULL, NULL, NULL
	EXEC PROC_VENDA_ESTAB 'I',4480, 1001, 100,NULL, NULL, NULL, NULL
	EXEC PROC_VENDA_ESTAB 'I',4480, 1002, 200,NULL, NULL, NULL, NULL

	EXEC PROC_VENDA_ESTAB 'P',3245, NULL, NULL,C, NULL, NULL, 1000
	EXEC PROC_VENDA_ESTAB 'P',4480, NULL, NULL,D, 15598, NULL, NULL

	INSERT INTO TB_ESTABELECIMENTOS VALUES (NEWID(), 'AGORA PAES II','99.141.683/0001-64', GETDATE())
	INSERT INTO TB_ESTABELECIMENTOS VALUES (NEWID(), 'AGORA E DEPOIS','51.245.830/0001-10', GETDATE())
	INSERT INTO TB_ESTABELECIMENTOS VALUES (NEWID(), 'AGORA DOANE IV','57.260.161/0001-31', GETDATE())
	...
	INSERT INTO TB_FUNCIONARIOS VALUES (NEWID(), 'GIOVANA','BRUNA ISADORA SILVEIRA', '49.052.192-7','104.911.713-15','24/06/1976', 'F', 'ADMINISTRADOR',NULL,GETDATE())
	INSERT INTO TB_FUNCIONARIOS VALUES (NEWID(), 'FABIO','EMANUEL LIMA','17.153.772-5','524.546.303-20','11/08/1981', 'M','GERENTE',NULL,GETDATE())
	INSERT INTO TB_FUNCIONARIOS VALUES (NEWID(), 'ELISA','YASMIN DA MATA','19.639.673-6','735.776.913-52','25/06/1981', 'F','BALCONISTA',NULL,GETDATE())
	INSERT INTO TB_LOGIN (USUARIO, SENHA) VALUES ('T', 'T')	  
SELECT * FROM TB_CONTAS_RECEBER 
SELECT * FROM TB_CONTAS_PAGAR 
SELECT * FROM VW_CONTAS_PAGAR  
SELECT * FROM VW_CONTAS_PAGAR WHERE ID_PAGAR = 1009
